...
![분류한 뒤의 메일함. 답해야 할 메일 여섯 건에 빨간 띠와 답장 필요, 처리 필요, 승인 대기, 일정 배지가 붙고, 광고와 자동발송은 흐리게 보인다](screenshots/store-1-classified.png)
...
![확인이 필요한 메일 여섯 건만 남기고 나머지를 접은 화면. 패널에 6건만 보는 중, 전체 보기가 보인다](screenshots/store-2-filtered.png)
...
![맨 위 두 메일을 열어 본 뒤의 화면. 두 메일은 띠가 빠지고 배지에 읽음이 붙었으며, 패널의 숫자가 4건으로 줄었다](screenshots/store-3-handled.png)
